...
export type SemanticColorName = 'Main' | 'Error' | 'Warning' | 'Success' | 'Blue'
export type TagColorName = 'Purple' | 'Magenta' | 'Lake-blue' | 'Cyan' | 'Green' | 'Red' | 'Orange' | 'Yellow' | 'Grey'

export type SemanticColorResult = Record<string, string>

export function generateNeutralSemanticColors(mode: ThemeMode = 'light'): SemanticColorResult {
...
export function generateBasicSemanticColors(mode: ThemeMode = 'light'): SemanticColorResult {
...
export function generateSemanticColors(colorName: SemanticColorName, mode: ThemeMode = 'light'): SemanticColorResult {
  ...
  if (mode === 'light') {
    ...
    const base: SemanticColorResult = {
      [`${prefix}Bg`]: `var(--yakit-colors-${colorName}-10)`,
      [`${prefix}Bg-Hover`]: `var(--yakit-colors-${colorName}-20)`,
      [`${prefix}Focus`]: `var(--yakit-colors-${colorName}-30)`,
      [`${prefix}Border`]: `var(--yakit-colors-${colorName}-40)`,
...
export function generateTagSemanticColors(colorName: TagColorName, mode: ThemeMode = 'light'): SemanticColorResult {
  const prefix = `--Colors-Use-${colorName}-`
  if (mode === 'light') {
    if (colorName === 'Grey') {
      return {
...
export function generateStatusSemanticColors(mode: ThemeMode = 'light'): SemanticColorResult {
